...
| 2 | Модель User и миграция БД | ⏳ Ожидает | - | - |
...
**Создание модели (TDD):**
- [ ] 🔴 RED: Написать тест для создания модели User
- [ ] 🟢 GREEN: Создать класс `User` в `db_models.py` с полями:
  - `user_id`, `username`, `first_name`, `last_name`
  - `bio`, `age`, `created_at`, `updated_at`
- [ ] 🔵 REFACTOR: Добавить type hints, docstring, индексы
- [ ] Добавить метод `__repr__` для удобного отображения

**Создание миграции:**
- [ ] Создать миграцию Alembic: `alembic revision --autogenerate -m "add users table"`
- [ ] Проверить сгенерированный код миграции
- [ ] Применить миграцию: `alembic upgrade head`
- [ ] Убедиться что таблица создана в PostgreSQL

**Миграция существующих пользователей:**
- [ ] Создать data-миграцию для извлечения user_id из таблицы `messages`
- [ ] Заполнить таблицу `users` базовыми данными для существующих пользователей:
...
- [ ] Проверить что все user_id из messages теперь есть в users
...
- [ ] Создать миграцию с тестовыми пользователями (для разработки)
- [ ] Добавить примеры пользователей с разными наборами данных
